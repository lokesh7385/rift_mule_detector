"""
Vercel Serverless Function — Money Mule Detection API (Robust Wrapper)
"""
try:
    import os
    import sys
    ...
    # ─────────────────────────────────────────────────────────────
    # We wrap heavy imports to catch specific library failures
    try:
        ...
        # Re-raise to be caught by the outer block, but with context
        raise ImportError(f"Missing Dependency: {str(e)} | Sys Path: {sys.path}")
    ...
    def health():
        return jsonify({
            'status': 'ok',
            'engine': 'MuleWatch Serverless (Robust)',
            'versions': {
...
# ─────────────────────────────────────────────────────────────
    ...
    try:
        ...
        # Emergency fallback if even Flask fails
        # This part might not work in Vercel as it expects 'app' to be a WSGI app,
        # but importing Flask usually works if it's installed.
        pass
